...
| 日期 | 版本 | 变更类型 | 主要变更 |
|------|------|---------|---------|
| 2026-01-23 | 2.2.0 | 最佳实践 | 新增"规则管理：三层防护体系"最佳实践 |
...
| 日期 | 版本 | 变更内容 | 原因 |
|------|------|---------|------|
| 2026-01-23 | 2.1.0 | 新增目录结构强制约束规则 | 防止错误的嵌套目录结构 |
...
| `scripts/` (根目录) | 脚本应按类型分类 | `main/backend/scripts/` |
| `src/` (根目录) | 代码应在 main 下 | `main/backend/` 或 `main/frontend/` |
| `backend/` (根目录) | 应在 main 下 | `main/backend/` |
| `frontend/` (根目录) | 应在 main 下 | `main/frontend/` |
| `main/backend/main/` | 错误的嵌套结构 | `main/` |
| `main/backend/docs/` | 文档应统一管理 | `main/docs/` |
| `main/frontend/main/` | 错误的嵌套结构 | `main/` |
| `main/frontend/docs/` | 文档应统一管理 | `main/docs/` |

#### 强制规则说明

1. **所有文档统一放 `main/docs/`**
   - 包括 PRD、技术设计、API 文档、测试报告等
   - 不允许在 `main/backend/docs/` 或 `main/frontend/docs/` 创建文档

2. **所有测试统一放 `main/tests/`**
   - 后端测试 → `main/tests/backend/`
   - 前端测试 → `main/tests/frontend/`
   - 集成测试 → `main/tests/integration/`

3. **禁止嵌套 `main/` 目录**
   - `main/backend/` 下不能有 `main/` 子目录
   - `main/frontend/` 下不能有 `main/` 子目录

4. **脚本文件位置**
   - 后端脚本 → `main/backend/scripts/`
   - 前端脚本 → `main/frontend/scripts/`（如果需要）
   - 通用脚本 → 根目录（如 `package.json` 中的 scripts）

#### 自动执行机制

这些约束通过 PreToolUse Hook 自动执行：
...
**核心原则**: 文档化（被动教育）+ 自动执行（主动防护）+ 快速参考（辅助查询）

#### 为什么需要三层？

| 层级 | 角色 | 职责 | 优势 | 局限 |
|------|------|------|------|------|
| **文档教育** (project_standards.md) | 单一事实来源 | 解释"为什么" | 全面、可搜索、可引用 | 依赖主动阅读 |
| **自动执行** (Hooks) | 自动执行器 | 执行"怎么做" | 实时拦截、零遗漏 | 无法解释原因 |
| **快速参考** (CLAUDE.md) | 快速指南 | 常见场景决策 | 简洁、实用、易查 | 不够全面 |

#### 实施策略

1. **文档先行**: 先在 project_standards.md 中定义规则和原因
2. **自动执行**: 在 Hooks 中实现自动检查和拦截
3. **错误引导**: 错误提示中引用文档位置，帮助开发者理解
4. **快速查询**: 在 CLAUDE.md 中提供简化版规则
5. **定期同步**: 规则变更时，同步更新三层内容
...
**详细说明**: 参考 `.claude/rules/system-design.md`
